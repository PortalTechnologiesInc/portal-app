...
export default function RecoverTicketsScreen() {
  ...
  const [mintUrls, setMintUrls] = useState<string[]>(['https://mint.getportal.cc']);
  ...
  const handleRecoverTickets = async () => {
    ...
    setIsLoading(true);
    try {
      mintUrls.forEach(async (url) => {
        // Attempt to recover tickets from this mint URL
        try {
          ...
          for (const set of mintKeys.keysets) {
            console.log(`Set: ${set.unit}`);
            const wallet = await addWallet(
              url,
              set.unit.toLowerCase()
            );
    ...
    <SafeAreaView style={[styles.safeArea, { backgroundColor: backgroundColor }]} edges={['top']}>
      <ThemedView style={styles.container}>
        ...
        <ScrollView style={styles.content} contentContainerStyle={styles.contentContainer}>
          ...
          {/* Loading State */}
          {isLoading && (
            <ThemedView style={[styles.loadingCard, { backgroundColor: cardBackgroundColor }]}>
